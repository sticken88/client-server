...
void upload_file(udp_socket *connection, linked_list *list, fd_manager *fd, struct sockaddr_in father_srv, char *file_name);
...
int main(int argc, char **argv){
   ...
   while(1){
      ...
      if(!list->is_empty()){ // list not empty, there are commands to process..
...
void upload_file(udp_socket *connection, linked_list *list, fd_manager *fd, struct sockaddr_in father_srv, char *file_name){
   ...
   char send_buffer[BUFLEN], receive_buffer[BUFLEN];
   char choice[] = "UP";
   struct timeval max_timeout, timeout;
   struct sockaddr_in child_srv;
   socklen_t father_srvlen, child_srvlen;   
   fd_set u_cset;
